...
class Ui_uygulamaPrototip(QWidget):
    ...
    def buttonClicked(self):   
            ...
            self.sitk_masked_image = sitk.GetImageFromArray(self.masked_image)
            image_dimension=3
            index=[15]*image_dimension
            itk_image = data_transforms.sitk_to_itk(self.sitk_masked_image, image_dimension, index)
    
            # Rendering
            self.vtk_img = data_transforms.vtk_image_from_image(itk_image)
            self.render(self.vtk_img)
            '''
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Nrrd Dosyası Bulunamadı!")
    ...
    def saveClicked(self):
        try:
             ...
             writer.SetFileName(self.fileName[:-5]+'.nhdr')
             writer.Execute(self.sitk_masked_image)
        except:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Kaydedilecek Dosya Bulunamadı!")
    ...
    def open3d(self):
        try:
            options = QFileDialog.Options()
            options |= QFileDialog.DontUseNativeDialog
            fileName, _ = QFileDialog.getOpenFileName(self,"Dosya Yükle", "","nhdr Files (*.nhdr)", options=options)
            sitk_masked_image = sitk.ReadImage(fileName)
            ...
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Warning)
            msg.setText("3 boyutlu model oluşturulamadı!")
    ...
    def setupUi(self, uygulamaPrototip):
        ...
        uygulamaPrototip.setWindowIcon(QtGui.QIcon('icon.png'))
        uygulamaPrototip.setFixedSize(955, 856)
        ...
        self.pushButton.setGeometry(QtCore.QRect(50, 670, 251, 111))
        self.savebutton = QtWidgets.QPushButton(self.centralwidget)
        self.savebutton.setGeometry(QtCore.QRect(310, 670, 290, 111))
        self.loadbutton = QtWidgets.QPushButton(self.centralwidget)
        self.loadbutton.setGeometry(QtCore.QRect(610, 670, 290, 111))
        font = QtGui.QFont()
        ...
        self.loadbutton.clicked.connect(self.open3d)
    ...
    def retranslateUi(self, uygulamaPrototip):
        ...
        uygulamaPrototip.setWindowTitle(_translate("uygulamaPrototip", "3D Akciğer"))
        self.pushButton.setText(_translate("uygulamaPrototip", "3 BOYUTLU GÖRÜNTÜLE"))
        self.savebutton.setText(_translate("uygulamaPrototip", "3 BOYUTLU MODELİ KAYDET"))
        self.loadbutton.setText(_translate("uygulamaPrototip", "3 BOYUTLU MODELİ YÜKLE"))
